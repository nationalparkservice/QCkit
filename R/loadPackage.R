...
#'
#' \code{loadPackage} reads the data file from a package and loads it into one or more data frames.
#'
#' @param fileName is a filename for the data package zip file.
#' @param directory is the directory where the data package sits.
...
#' @examples
#'
#' ParkTandETaxa<-loadDataPackage("file.zip","data/final",dataFormat="csv",metadataFormat="eml")


loadPackage <-function(fileName,directory,dataFormat,metadataFormat,features=NULL){
  
  if (dataFormat=="csv" & metadataFormat=="eml") {
    
    fileList<-zip_list(fileName)
    csvfile <- as.character(subset(fileList, grepl(".csv",filename)))
    emlfile <- as.character(subset(fileList, grepl(".xml",filename)))
    csvFilename <- paste(directory,"/data_objects/",csvfile[1],sep="")
    emlFilename <- paste(directory,"/eml/",emlfile[1],sep="")
    ...
    assign(paste0(fileName,"_data"),workingdatafile, envir=.GlobalEnv)
