...
#' @param directory String. Path to the file(s) to have blanks replaced with NAs. Defaults to the working directory of the project (here::here())
...
replace_blanks <- function(directory = here::here()) {
  ...
  my_data <- lapply(my_path, function(x) readr::read_csv(x, show_col_types=FALSE))
  ...
  for (i in 1:length(seq_along(my_data))) {
